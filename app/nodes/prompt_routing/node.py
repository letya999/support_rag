...
async def route_prompt_node(state: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    # We construct a text block that will be injected as the 'system' message or part of it.
...
def _format_history(history: List[Dict[str, Any]]) -> str:
    if not history:
        return ""
    # Take last 3 messages (excluding current one if it's already there? usually history tracks past)
    recent = history[-3:]
    lines = []
    for msg in recent:
