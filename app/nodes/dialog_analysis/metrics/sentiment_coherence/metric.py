...
class SentimentCoherence(DialogAnalysisBaseMetric):
    """LLM-as-judge metric to evaluate sentiment coherence with conversation context."""
    ...
    def calculate(self, expected: Any, actual: Any, **kwargs) -> float:
        """
        Calculate sentiment coherence score.
        ...
        if not question or not sentiment:
            return 0.0
        ...
        try:
            ...
            score = result.get("overall_coherence", 0.0)
            
            # Validate score range
            return max(0.0, min(1.0, float(score)))
            
        except Exception as e:
            logger.error("SentimentCoherence evaluation failed", extra={"error": str(e)})
            return 0.0
