...
### Опциональные Headers:
```bash
-H "X-Request-ID: req_unique_id"     # Генерируется автоматически, но можно передать свой
-H "X-User-ID: user_123"              # Опционально для логирования и метаданных
```

### ℹ️ Примечание по безопасности
- **Текущее состояние:** Система работает в **закрытой корпоративной сети**
- **Аутентификация:** Не требуется (сеть уже защищена)
- **Авторизация:** Не требуется (внутренние сервисы имеют доступ ко всем эндпоинтам)
- **Будущее:** При необходимости расширения на внешние сети можно добавить Bearer token аутентификацию
